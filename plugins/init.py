...
class main:
    ...
    def execute(self, vk : VK, peer : int, **mess):
        if "-init" in sys.argv:
            db.execute("CREATE TABLE IF NOT EXISTS moders   (vk_id INT NOT NULL PRIMARY KEY, event INT DEFAULT 0, days_without_posts INT DEFAULT 0, money_left INT DEFAULT 0, UNIQUE(vk_id))")
